...
static int gettimeout(struct timeval end, struct timeval *timeoutp)
{
    ...
    if (timeoutp->tv_usec >= MILLON)
    {
        timeoutp->tv_sec++;
        timeoutp->tv_usec -= MILLON;
    }
...
int r_close(int fileds)
{
    int retval;
    while ((retval = close(fileds)) == -1 && errno == EINTR)
        ;
    return retval;
}

int r_dup2(int fileds, int fileds2)
{
    int retval;
    while ((retval = dup2(fileds, fileds2)) == -1 && errno == EINTR)
        ;
    return retval;
}
...
{
    int retval;
    while ((retval = open(path, oflag, mode)) == -1 && errno == EINTR)
        ;
    return retval;
}
...
struct timeval add2currenttime(double seconds)
{
    ...
    newtime.tv_usec += (int)((seconds - (int)seconds) * D_MILLON + 0.5);
    if (newtime.tv_usec >= MILLON)
    {
        newtime.tv_sec++;
        newtime.tv_usec -= MILLON;
    }
    return newtime;
}
